...
/**
 * This class represents the subcommand 'calls' to list all method calls.
 * 
 * @author flaxel
 * @since 1.0.0
 */
@Command(name = "calls", description = "list all method calls", sortOptions = true)
public class ListMethodCallsCommand implements Runnable {
	...
	/**
	 * true if the annotations should be ignored, otherwise false
	 */
	@Option(names = { "--ignore", "-i" }, description = "ignore annotations")
	...
	public void run() {
		...
		ListMethodCallHandler handler = new ListMethodCallHandler(Console.getOutput())
				.ignoringAnnotations(ignoringAnnotations);
